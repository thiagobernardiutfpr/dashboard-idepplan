...
func sharedStrings(zr *zip.ReadCloser) []string { raw,err:=readZipFile(zr,"xl/sharedStrings.xml");if err!=nil{return nil};var table sst;if xml.Unmarshal(raw,&table)!=nil{return nil};out:=make([]string,0,len(table.Items));for _,item:=range table.Items{text:=item.Text;for _,run:=range item.Runs{text+=run.Text};out=append(out,text)};return out }
func colIndex(ref string) int { n:=0;for _,r:=range ref{if r<'A'||r>'Z'{break};n=n*26+int(r-'A'+1)};return n-1 }
func cellText(c sheetCell, shared []string) string { switch c.Type{case"s":i,_:=strconv.Atoi(c.Value);if i>=0&&i<len(shared){return shared[i]};case"inlineStr":text:=c.Inline.Text;for _,run:=range c.Inline.Runs{text+=run.Text};return text;default:return c.Value};return "" }
func readSheet(raw []byte,shared []string)([][]string,error){var ws worksheet;if err:=xml.Unmarshal(raw,&ws);err!=nil{return nil,err};rows:=make([][]string,0,len(ws.Rows));for _,row:=range ws.Rows{max:=-1;values:=map[int]string{};for _,c:=range row.Cells{idx:=colIndex(c.Ref);if idx<0{continue};values[idx]=cellText(c,shared);if idx>max{max=idx}};if max<0{rows=append(rows,nil);continue};out:=make([]string,max+1);for i,v:=range values{out[i]=v};rows=append(rows,out)};return rows,nil}
...
func parseVerticalRows(rows [][]string)([]AtendeProcess,bool){header,pc,sc,rc,dc:=-1,0,1,4,9;limit:=len(rows);if limit>20{limit=20};for i:=0;i<limit;i++{for j,c:=range rows[i]{n:=normalizeLabel(c);if n=="processo"{pc=j};if n=="situacao"{sc=j}};foundP,foundS:=false,false;for _,c:=range rows[i]{n:=normalizeLabel(c);if n=="processo"{foundP=true};if n=="situacao"{foundS=true};if n=="requerente"{rc=indexOf(rows[i],c)};if n=="data abertura"||n=="data de abertura"{dc=indexOf(rows[i],c)}};if foundP&&foundS{header=i;break}};if header<0{return nil,false};var out []AtendeProcess;var cur *AtendeProcess;finish:=func(){if cur==nil{return};enrichProperty(cur);out=append(out,*cur);cur=nil};for _,row:=range rows[header+1:]{proc:=rowValue(row,pc);if processPattern.MatchString(proc){finish();cur=&AtendeProcess{ProcessNumber:proc,Applicant:rowValue(row,rc),Status:rowValue(row,sc),OpenedAt:parseDate(rowValue(row,dc))};continue};if cur==nil{continue};for i,c:=range row{lab:=normalizeLabel(c);switch{case lab==labelAssunto:cur.Subject=rightValue(row,i);case lab==labelSubassunto:cur.Subsubject=rightValue(row,i);case lab==labelObservacao||lab=="observacoes gerais:":v:=rightValue(row,i);if v!=""{if cur.Observation!=""{cur.Observation+=" "};cur.Observation+=v};case strings.Contains(lab,"data/hora encerramento:")||strings.Contains(lab,"data hora encerramento:"):cur.ClosedAt=parseDate(rightValue(row,i));case strings.Contains(lab,"motivo encerramento:"):cur.CloseReason=rightValue(row,i)}}};finish();filtered:=make([]AtendeProcess,0,len(out));unique:=map[string]AtendeProcess{};for _,p:=range out{old,ok:=unique[p.ProcessNumber];if !ok||completeness(p)>completeness(old){unique[p.ProcessNumber]=p}};keys:=make([]string,0,len(unique));for k:=range unique{keys=append(keys,k)};sort.Strings(keys);for _,k:=range keys{filtered=append(filtered,unique[k])};return filtered,true}
